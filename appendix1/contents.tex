...
\begin{figure}[htbp]
    ...
    \caption{Sample image: when its extension is pdf.} % 必ずlabelよりも上にcaptionを定義すること
    \label{fig:sample_pdf} % 必ずcaptionよりも下にlabelを定義すること
\end{figure}

labelで指定することで, \figref{fig:sample_pdf}のように参照することができる.labelの定義場所はfigureの一番下にしないと、参照の数字が変なことになる．captionでは [] が目次に示される内容で, \{\} が本文に示される内容である. 同じでいい場合は \{\} のみで良い.
